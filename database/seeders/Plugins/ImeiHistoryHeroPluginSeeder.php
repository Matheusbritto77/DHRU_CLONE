...
class ImeiHistoryHeroPluginSeeder extends AbstractPluginSeeder
{
    protected function slug(): string
    {
        return 'imei-history-hero';
    }

    protected function definition(): array
    {
        return [
            'name' => 'Hero Historico IMEI',
            'type' => 'custom',
            'is_system' => true,
            'icon' => 'heroicon-o-clock',
            'description' => 'Cabecalho do historico IMEI.',
            'default_settings' => [
                'eyebrow' => 'Historico IMEI',
                'title' => 'Acompanhe cada retorno com clareza.',
                'description' => 'Visualize o andamento dos pedidos, custos, referencias e respostas recebidas em um painel mais limpo.',
            ],
            ...
            <div class="mt-4 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                ...
                <div class="grid grid-cols-2 gap-3 sm:min-w-[280px]">
                    <div class="rounded-[24px] px-5 py-4 theme-soft">
                        ...
                        <p class="mt-3 text-2xl font-semibold theme-text">{{ $imeiOrders->total() ?? 0 }}</p>
                    </div>
                    <div class="rounded-[24px] px-5 py-4 theme-soft">
                        <p class="text-[11px] uppercase tracking-[0.25em] theme-muted">Pagina</p>
                        <p class="mt-3 text-2xl font-semibold theme-text">{{ $imeiOrders->currentPage() ?? 1 }}</p>
                    </div>
                </div>
            </div>
        </div>
