...
class ImeiHistoryTablePluginSeeder extends AbstractPluginSeeder
{
    protected function slug(): string
    {
        return 'imei-history-table';
    }

    protected function definition(): array
    {
        return [
            'name' => 'Tabela Historico IMEI',
            'type' => 'custom',
            'is_system' => true,
            'icon' => 'heroicon-o-table-cells',
            'description' => 'Tabela principal do historico IMEI.',
            'default_settings' => [
                'title' => 'Pedidos IMEI',
            ],
        ...
        <div class="rounded-[32px] p-6 md:p-8 theme-panel">
            <div class="flex items-center justify-between gap-4 flex-wrap">
                <div>
                    ...
                    <h2 class="mt-3 text-2xl font-semibold tracking-tight theme-text">Historico completo das suas ordens IMEI.</h2>
                </div>
                    ...
                    <table class="min-w-full">
                        <thead style="background: var(--theme-surface-soft);">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-[11px] font-semibold uppercase tracking-[0.28em] theme-muted">Resposta</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($imeiOrders as $order)
                                ...
                                <tr style="border-top: 1px solid var(--theme-border);">
                                    ...
                                    <td class="px-6 py-4 text-sm theme-muted"><div>{!! $order->api_response !!}</div></td>
                                </tr>
                            @empty
                                <tr>
            ...
            @if (method_exists($imeiOrders, 'links'))
                <div class="mt-6">
                    {{ $imeiOrders->links() }}
                </div>
            @endif
